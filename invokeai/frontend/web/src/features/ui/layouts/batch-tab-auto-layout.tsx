...
const mainPanelComponents: AutoLayoutDockviewComponents = {
  [LAUNCHPAD_PANEL_ID]: withPanelContainer(BatchLaunchpadPanel),
...
const leftPanelComponents: AutoLayoutGridviewComponents = {
  [SETTINGS_PANEL_ID]: withPanelContainer(BatchTabLeftPanel),
};
...
export const BatchTabAutoLayout = memo(() => {
  const onReady = useCallback<IGridviewReactProps['onReady']>(({ api }) => {
    initializeRootPanelLayout('batch', api);
  }, []);

  useEffect(
    () => () => {
      navigationApi.unregisterTab('batch');
    },
    []
  );

  return (
    <AutoLayoutProvider tab="batch">
      <GridviewReact
        className="dockview-theme-invoke"
...
BatchTabAutoLayout.displayName = 'BatchTabAutoLayout';
